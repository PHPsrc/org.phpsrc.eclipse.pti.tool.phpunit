...
public class RunTestCaseWithCodeCoverageAction extends ResourceAction {
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					for (IResource resource : resources) {
						...
						if (resource instanceof IFolder) {
							validator.validateFolder((IFolder) resource, true);
						} else if (resource instanceof IFile) {
							IFile file = (IFile) resource;
							if (PHPUnit.isTestSuite(file)) {
								validator.validateTestSuite(file, true);
							} else {
								IFile testCaseFile = PHPUnit
										.searchTestCase(file);
